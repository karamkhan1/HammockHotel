...
public class HammockHotel {
    ...
    public boolean addUnique(String guestName) {
        // Initialize unique assumption
        boolean unique = true;
        // Checks each guest in each node and checks to
        // see if whether that guest matches with the
        // proposed addition.
        for(int i = 0; i < this.rooms.length; i++) {
            Node check = this.rooms[i];
            while(unique && (check != null)) {
                if(guestName.equals(check.data)) {
                    unique = false;
                }
                check = check.next;
            }
        }
        // If the name is unique, add it.
        if(unique) {
            add(guestName);
        }
        // Return unique boolean.
        return unique; // Dummy code ... to be replaced by your code
    }  // method addUnique
    ...
    public boolean isEmpty() {
        // Checks each Node and if a single node is occupied
        // return false.
        for(int i = 0; i < this.rooms.length; i++) {
            Node check = this.rooms[i];
            if(check != null) {
        ...
        // Not one node is occupied therefore return true;
        return true;
    }  // method isEmpty
    ...
    public String[] toArray() {
        // Initializes the String array with respective size
        String[] toArray = new String[this.usage];
        // Array index - iterator
        int z = 0;
        // Traverses through each node then traverses through
        // each guest in the node and adds each guest in the
        // string array.
        for(int i = 0; i < this.rooms.length; i++) {
            Node check = this.rooms[i];
            while(check != null) {
                toArray[z] = check.data;
                z++;
                check = check.next;
            }
        }
        // Returns toArray array
        return toArray; // Dummy code ... to be replaced by your code
    }  // method toArray
     ...
     */
    public void clear() {
        // Traverses through each hammock in the hotel
        for(int i = 0; i < this.rooms.length; i++) {
            // If this hammock is occupied, then empty it
            if(this.rooms != null) {
                this.rooms[i] = null;
            }
     ...
     */
    public int[] stats() {
        // Initializes stats array with respective size
        int[] stats = new int[this.rooms.length];
        // Guest counter
        int z = 0;
        // Checks each node and counts each guest housed in
        // each node and stores that value in the stats array
        for(int i = 0; i < this.rooms.length; i++) {
            Node check = this.rooms[i];
            while(check != null) {
                z++;
        ...
        // Returns stats array
        return stats; // Dummy code ... to be replaced by your code
    }  // method stats
